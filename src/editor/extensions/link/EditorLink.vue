      ...
      <template #floating>
        <div v-if="isEdit" class="w-100 p-2">
          ...
          <div class="mt-4 flex items-center justify-end gap-2">
            <button class="cursor-pointer rounded border-none bg-blue-600 px-2 py-0.5 text-sm text-white transition-colors hover:bg-blue-700 !disabled:cursor-not-allowed !disabled:bg-slate-400" :disabled="!canUpdate" @click="updateLink">
              Update
            </button>
        ...
        <div v-else class="h-8 flex items-center gap-1">
          <a class="inline-block max-w-80 cursor-pointer truncate rounded-1.5 px-2 py-0.5 text-sm text-blue-600 transition-colors hover:bg-zinc-300 dark:hover:bg-zinc-700" :href="node.attrs.href" target="_blank">{{ node.textContent }}</a>
          <div title="Edit link" class="cursor-pointer rounded-2 p-1 text-base transition-colors hover:bg-zinc-300 dark:hover:bg-zinc-700" @click="toEdit">
            <div class="i-mdi:edit-outline h-1em w-1em" />
          </div>
          <div title="Unlink" class="cursor-pointer rounded-2 p-1 text-base transition-colors hover:bg-zinc-300 dark:hover:bg-zinc-700" @click="unsetLink">
            <div class="i-tdesign:link-unlink h-1em w-1em" />
          </div>
          <div title="Delete link" class="cursor-pointer rounded-2 p-1 text-base transition-colors hover:bg-zinc-300 dark:hover:bg-zinc-700" @click="deleteNode()">
