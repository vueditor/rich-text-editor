    ...
    <EditorFloating ref="floatingRef" mode="hover" :disabled="!editor.isEditable" @open="resetState">
      ...
      <template #floating>
        ...
        <div v-else class="h-8 flex items-center gap-2">
          <a class="inline-block max-w-80 cursor-pointer truncate rounded-2 px-2 py-0.5 text-sm transition-colors hover:bg-color-hover-soft" :href="node.attrs.href" target="_blank">{{ node.textContent }}</a>
